...
public class OfferSender extends TimerTask {
    ...
    public OfferSender(long sendingTime, SmartMeter smartMeter, Credentials credentials, ArrayList<String> urls, int round, int supplierID, BigInteger energyAmount, BigInteger priceRate){
        this.ID=supplierID;
    ...
    public void run() {
        ...
        try {
          //  System.out.println("Producer: "+ID+" Bidding State: "+poolMarketContract.getState().send().toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
        try {
            submissionTime=System.currentTimeMillis();
            transactionReceipt=poolMarketContract.submitEnergyOffer(this.energyAmount,this.priceRate).send();
           ...
           // System.out.println("Supplier "+ ID + " Energy Offer Submitted for round " +round+ " Submission Time: "+submissionTime);
        } catch (Exception e) {
            responseTime=System.currentTimeMillis();
           // e.printStackTrace();
